...
Processor.prototype.asset_host = function(filepath, done) {
  if (typeof this.options.asset_host !== 'function') {
    done(new Error('asset_host should be a function'));
  }
  this.options.asset_host(filepath, function(host) {
    done(null, url.resolve(host, filepath));
  });
};
...
Processor.prototype.image_width = function(filepath, done) {
  var file = this.real_path(filepath, 'images')
  if(typeof done !== 'function') {
    return sizeOf(file).width
  }
  sizeOf(file, function(err, dimensions) {
    done(err, dimensions.width)
  })
};
...
Processor.prototype.image_height = function(filepath, done) {
  var file = this.real_path(filepath, 'images')
  if(typeof done !== 'function') {
    return sizeOf(file).height
  }
  sizeOf(file, function(err, dimensions) {
    done(err, dimensions.height)
  })
};
...
Processor.prototype.inline_image = function(filepath, mime_type, done) {
  ...
  var dataUrl = 'data:' + mime_type + ';base64,' + data.toString('base64')
  if(typeof done !== 'function') {
    return dataUrl;
  }
  done(null, dataUrl);
...
Processor.prototype.asset_url = function(filepath, segment, done) {
  ...
  var restoreFragment = function(err, url) {
    done(err, url + fragment);
  };
  ...
  var next = function(http_path) {
    if (this.options.asset_host) {
      ...
      restoreFragment(null, http_path);
    }
  }.bind(this);
  ...
  if(sync) {
    deasync.loopWhile(function() { return typeof data === 'undefined'; });
    return data
  }
};

Processor.prototype.image_url = function(filepath, done) {
  return this.asset_url(filepath, 'images', done);
};

Processor.prototype.font_url = function(filepath, done) {
  return this.asset_url(filepath, 'fonts', done);
};
...
Processor.prototype.font_files = function(files, done) {
  ...
  if(typeof done !== 'function') {
    var sync = true
    done = function(err, processed_files) {
